...
class ForgeMC {
    ...
    async downloadInstaller(Loader) {
        let metaData = (await (0, node_fetch_1.default)(Loader.metaData).then(res => res.json()))[this.options.loader.version];
        ...
        if (this.options.loader.build === 'latest') {
            ...
            let promotion = await (0, node_fetch_1.default)(Loader.promotions).then(res => res.json());
        ...
        let meta = await (0, node_fetch_1.default)(Loader.meta.replace(/\${build}/g, metaData)).then(res => res.json());
